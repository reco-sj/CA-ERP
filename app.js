...
function renderDashboardTable() {
  ...
  tbody.innerHTML = rows.map(([id, c]) => `
    <tr>
      <td class="cell-strong td-title">${esc(c.companyName)}
        ${c.gstin ? `<br><span class="cell-muted">GSTIN: ${esc(c.gstin)}</span>` : ''}
        ${c.pan ? `<br><span class="cell-muted">PAN: ${esc(c.pan)}</span>` : ''}</td>
      <td data-label="Authorised person">${esc(c.authorisedPerson)}</td>
      <td data-label="Contact">${esc(c.mobile)}${c.email ? `<br><span class="cell-muted">${esc(c.email)}</span>` : ''}</td>
      <td data-label="City">${esc(c.city)}</td>
      <td data-label="Task progress">${renderProgressChips(c)}</td>
      <td class="admin-only cell-strong" data-label="Total fees">${inr(totalFees(c))}</td>
      <td class="admin-only" data-label="Payment">${renderPaymentPill(c)}</td>
    </tr>
  `).join('');
}
  ...
  $('#client-tbody').innerHTML = rows.map(([id, c]) => {
    ...
    <tr class="${open ? 'row-open' : ''}">
      ...
      <td class="cell-strong td-title">${esc(c.companyName)}
        ${c.gstin ? `<br><span class="cell-muted">GSTIN: ${esc(c.gstin)}</span>` : ''}
        ${c.pan ? `<br><span class="cell-muted">PAN: ${esc(c.pan)}</span>` : ''}</td>
      <td data-label="Authorised person">${esc(c.authorisedPerson)}
        ${c.email ? `<br><span class="cell-muted">${esc(c.email)}</span>` : ''}</td>
      <td data-label="Mobile">${esc(c.mobile)}</td>
      <td data-label="City">${esc(c.city)}</td>
      ...
      <td class="admin-only cell-strong" data-label="Total fees">${inr(totalFees(c))}</td>
      <td class="admin-only" data-label="Payment">
        <div class="pay-cell">
          ${renderPaymentPill(c)}
      ...
      <td class="admin-only td-actions" data-label="Actions">
...
function renderUserTable() {
  ...
  $('#user-tbody').innerHTML = entries.map(([username, u]) => {
    ...
    return `
    <tr>
      <td class="cell-strong td-title">${esc(username)}${isSelf ? ' <span class="cell-muted">(you)</span>' : ''}</td>
      <td data-label="Display name">${esc(u.displayName || '—')}</td>
      <td data-label="Role"><span class="role-badge ${u.role === 'staff' ? 'staff' : ''}" style="position:static">${esc(u.role)}</span></td>
